import { Tab } from '@headlessui/react';
import Head from 'next/head';
import Link from 'next/link';

const tabs = [
  {
    key: 'all',
    display: 'All',
  },
  {
    key: 'oceans',
    display: 'Oceans',
  },
  {
    key: 'forests',
    display: 'Forests',
  },
]

export default function Home() {
  return (
    <div className="flex flex-col h-full bg-[url('/twodudesstaringatthesky.jpg')] bg-top bg-cover">
      <Head>
        <title>Nature Canvas</title>
        <meta name='description' content='Generated by create next app' />
        <meta name='viewport' content='width=device-width, initial-scale=1' />
        <link rel='icon' href='/favicon.ico' />
      </Head>
      <header className='flex justify-between items-center h-[90px] px-28'>
        <div className=''>Nature Canvas</div>
        <Link href='#' className='rounded-3xl bg-white text-stone-700 px-6 py-2 hover:bg-opacity-90'>
            Contact
        </Link>
      </header>
      <main className='grow'>
        <div className="flex flex-col items-center h-full">
      <Tab.Group>
      <Tab.List className='flex items-center gap-16'>
        {tabs.map(tab => (
        <Tab key={tab.key} className='p-2'>
          {({selected}) => (<span className={selected ? 'text-white' : 'text-stone-600'}>{tab.display}</span>)}
        </Tab>
        ))}
      </Tab.List>
      <Tab.Panels className='h-full bg-black bg-opacity-80 max-w-[900px] w-full p-2 sm:p-4 text-slate-700'>
        <Tab.Panel>All Photos</Tab.Panel>
        <Tab.Panel>Oceans</Tab.Panel>
        <Tab.Panel>Forests</Tab.Panel>
      </Tab.Panels>
    </Tab.Group>
        </div>
      </main>
      <footer className='h-[60px] flex justify-center items-center'>
        <p>Nature Canvas</p>
      </footer>
    </div>
  );
}
